...
// Records a new event with the current local time at the top of the log
// (index 0), trimming the oldest entry once past EVENTS_MAX. Updates the
// Events window immediately if it's currently open; otherwise the entry just
// sits in eventsList until the window is next opened (Events_Repopulate on
// WM_CREATE picks it up).
static void Events_AddEvent(const std::string& text, COLORREF color = 0) {
    time_t now = time(0);
    struct tm ltm = {};
    localtime_s(&ltm, &now);
    char buf[16];
    ...
    eventsList.push_front(EventEntry{ std::string(buf), text, color });
...
// Alert-fired event: "SYMBOL: Alert UP/DOWN at PRICE", colored green/red to
// match the rest of the app's up/down conventions. Called from dashboard.h's
// WM_SHOW_ALERT handler, the single place both live and quick-fired alerts
// pass through.
static void Events_AddAlertEvent(int conId, const std::string& symbol, double price, bool isUp) {
    std::string text = symbol + ": Alert " + (isUp ? "UP" : "DOWN") + " at " + FormatFixed(price, 2);
    Events_AddEvent(text, isUp ? COINS_CLR_GREEN : COINS_CLR_RED);
}
